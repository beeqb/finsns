...
def get_stock(stock, crawler, errfile, elock, s_page, delta):
    print('Start to fetch %s:...' % stock)
    s_t = time.time()
    guba = GuBa(stock, crawler, '2018-1-1', '2018-5-31', errfile, elock, s_page, delta)
    tiezis = guba.run()
...
def main():
    ...
    pars = pool.map(lambda c: get_stock(c, crawler, perrfile, elock, 1, 10), codes)
    # for stock in codes:
